...
export class GenerationHistoryService {
    ...
    static async getUserHistory(userId, page = 1, limit = 20) {
        try {
            ...
            // 获取历史记录列表 - 使用简化查询避免参数绑定问题
            const listQuery = `SELECT id, user_id, prompt, original_images, generated_image, 
       ...
       LIMIT ${limit} OFFSET ${offset}`;
            console.log('getUserHistory - listQuery:', listQuery);
            const [histories] = await pool.execute(listQuery);
    ...
    static async getUserStats(userId) {
        try {
           ...
           COALESCE(SUM(CASE WHEN status = 'completed' THEN 1 ELSE 0 END), 0) as successful_generations,
           COALESCE(SUM(CASE WHEN status = 'failed' THEN 1 ELSE 0 END), 0) as failed_generations,
           COALESCE(SUM(CASE WHEN status = 'completed' THEN COALESCE(consumed_times, 0) ELSE 0 END), 0) as total_consumed_times
         FROM generation_history 
         WHERE user_id = ${userId}`);
            const result = {
    ...
    static async getAllHistory(page = 1, limit = 20, filters) {
        try {
            ...
            console.log('getAllHistory - whereClause:', whereClause);
            console.log('getAllHistory - params:', params);
            // 获取总数和统计信息
            const [countResult] = await pool.execute(`SELECT 
           COUNT(*) as total,
           COALESCE(SUM(CASE WHEN gh.status = 'completed' THEN 1 ELSE 0 END), 0) as successful_generations,
           COALESCE(SUM(CASE WHEN gh.status = 'failed' THEN 1 ELSE 0 END), 0) as failed_generations,
           COALESCE(SUM(CASE WHEN gh.status = 'completed' THEN COALESCE(gh.consumed_times, 0) ELSE 0 END), 0) as total_consumed_times
         ...
         ${whereClause}`, params);
            ...
            // 获取历史记录列表 - 使用简化查询
            let listQuery = `SELECT gh.*, u.username, u.email 
         FROM generation_history gh 
         LEFT JOIN users u ON gh.user_id = u.id 
         ${whereClause}
         ORDER BY gh.created_at DESC 
         LIMIT ${limit} OFFSET ${offset}`;
            console.log('getAllHistory - listQuery:', listQuery);
            const [histories] = await pool.execute(listQuery, params);
            console.log('getAllHistory - 查询结果数量:', histories.length);
            // 处理JSON字段
            const processedHistories = histories.map(history => {
                if (history.original_images && typeof history.original_images === 'string') {
                    try {
            ...
            console.error('getAllHistory 数据库操作失败:', error);
