...
export class PackageService {
    ...
    static async getPackages(query = {}) {
        ...
        // 获取数据
        const [rows] = await pool.execute(`SELECT * FROM packages ${whereClause} ${orderClause} LIMIT ${limit} OFFSET ${offset}`, queryParams);
        // 为前端兼容性添加 usage_count 字段
        const processedItems = rows.map(item => ({
            ...item,
            usage_count: item.times, // 前端兼容字段
            is_active: item.status === 'active' // 前端兼容字段
        }));
        return {
    ...
    static async getActivePackages() {
        const [rows] = await pool.execute('SELECT * FROM packages WHERE status = ? ORDER BY sort_order ASC, price ASC', ['active']);
        // 为前端兼容性添加 usage_count 字段
        const processedItems = rows.map(item => ({
            ...item,
            usage_count: item.times, // 前端兼容字段
    ...
    static async createPackage(packageData) {
        const { name, usage_count, validity_days, price, sort_order = 0 } = packageData;
        // 检查套餐名称是否已存在
        ...
        // 插入新套餐
        const [result] = await pool.execute(`INSERT INTO packages (name, times, validity_days, price, sort_order) 
       VALUES (?, ?, ?, ?, ?)`, [name, usage_count, validity_days, price, sort_order]);
    ...
    static async updatePackage(id, updateData) {
        ...
        Object.entries(updateData).forEach(([key, value]) => {
            if (value !== undefined) {
                // 处理字段映射
                const dbField = key === 'usage_count' ? 'times' : key;
    ...
    static async activatePackage(userId, packageId) {
        ...
        try {
         ...
         VALUES (?, ?, ?, ?, 'active')`, [userId, packageId, packageInfo.times, expiresAt]);
            // 升级用户为会员
            await this.upgradeMember(userId, expiresAt);
